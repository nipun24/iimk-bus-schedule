...
const STUDENT = [
  ...
  [
    "21:30",
    ["PGP Auditorium", "Phase V Campus", "C&D Housing", "PGP Auditorium"],
    "Returns to PGP",
  ],
...
const STAFF = [
  ...
  [
    "08:42",
    ["Main Office", "Phase V Apt", "Resi Hill", "Main Office"],
    "Resi Hill & back",
  ],
  [
    "10:15",
    ["Main Office", "Phase V Apt", "Apt-1", "Resi Hill", "Main Office"],
    "& back",
  ],
  ...
  [
    "13:05",
    ["Main Office", "Phase V Apt", "Resi Hill", "Main Gate"],
    "Via Phase-V Apt, R-Hill",
  ],
  [
    "13:50",
    ["Main Gate", "Resi Hill", "Phase V Apt", "Main Office"],
    "Via R-Hill, Phase-V",
  ],
  ...
  [
    "20:30",
    ["Main Gate", "Phase V Apt", "Resi Hill", "Main Office"],
    "Via Phase-V, R-Hill",
  ],
...
function generateICS(request) {
  ...
  for (let off = 0; off <= LOOKAHEAD_DAYS; off++) {
    const base = istParts(
      new Date(istToEpoch(today.y, today.mo, today.d, 0, 0) + off * 86_400_000),
    );
  ...
  for (const { tr, startMs } of chosen) {
    ...
    const summary = `🚌: ${first} → ${last} (${tr.svc.toUpperCase()})`;
    let desc = `${svcLabel} shuttle\nRoute: ${tr.stops.join(" → ")}`;
    if (tr.note) desc += `\n${tr.note}`;
    desc += `\nDeparts ${first} at ${time12(tr.t)}`;
    desc += `\n\nhttps://iimkbus.nipunh.com`;
    ...
    L.push(fold("LOCATION:" + esc(tr.stops.join(" → "))));
    L.push("SEQUENCE:0");
